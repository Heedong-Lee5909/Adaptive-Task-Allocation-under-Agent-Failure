...
for j = 1:numTasks
    ...
    uavQueue{selectedUAV}(end+1) = j;
    taskAssigned(j) = true;
end 

for i = 1:numUAV
    disp(['UAV ', num2str(i), ' Initial Queue']);
    disp(uavQueue{i});
end 
    ...
    if simStep == 100
        ...
        for n = 1:length(failedTasks)
            ...
            for k = 1:numUAV
                ...
                disp(['UAV', num2str(k), ...
                    ' DistanceCost = ', num2str(distanceCost), ...
                    ', WorkloadCost = ', num2str(workloadCost), ...
                    ', TotalCost = ', num2str(cost)]);
            ...
            if selectedUAV ~= -1
                ...
                disp(['Task ', num2str(failedTask), ...
                    ' reassigned to UAV ', ...
                    num2str(selectedUAV)]);
    
                uavQueue{selectedUAV}(end+1) = failedTask;
    
                taskAssigned(failedTask) = true;

                for m = 1:numUAV
                    disp(['UAV ', num2str(m), ' Queue = ']);
                    disp(uavQueue{m});
                end
